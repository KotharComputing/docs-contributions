...
- The system-environment coupling is weak enough for a perturbative treatment;
- The bath can still be treated as an external environment, rather than as a fully dynamical part of the system we keep explicitly;
...
:::note
The choice of row-stacking is not physically or mathematically special. It is a choice of basis for the \(d^2\)-dimensional vector space of operators. What matters is using one convention consistently. In practice, this choice can still have numerical consequences, especially when one later compares different sparse or structured representations. The validation files check that the symbolic Liouville-space action agrees with the direct dense GKLS equation under the convention used here.
:::
...
where $\mathcal{L}$ is the generator of the vectorized GKLS dynamics. In this tutorial, I will often call this full generator the **Liouvillian**, since it is the object generating time evolution in Liouville space. Equivalently, one may call the full GKLS generator the **Lindbladian**, especially when emphasizing the dissipative terms. When needed, I will refer to the purely Hamiltonian contribution as the coherent or Hamiltonian part of the generator.
...
The implementation is centered around `liouville-core.aleph`.
...
1. `liouville-core.aleph` defines the reusable Liouville-space machinery.
2. `liouville-validation.aleph` checks the convention against direct dense GKLS calculations.
...
The file `liouville-validation.aleph` contains the reference checks. For example, the direct dense GKLS right-hand side is implemented as
...
The executable file `01-single-qubit-validation.aleph` simply runs the compact validation suite:

```aleph
include "gkls-vec-tutorial/liouville-validation.aleph"
...
The symbolic Liouvillian is assembled directly from the validated helpers in `liouville-core.aleph`:
...
![Driven dissipative qubit populations](./figures/single-qubit-populations-omega1-gamma0-3-dt0-02-steps1000.png)
...
![Driven dissipative qubit trace and Z expectation](./figures/single-qubit-trace-z-omega1-gamma0-3-dt0-02-steps1000.png)
...
![Closed vs open total $Z$](./figures/stationary-chain-total-z-n10-jz1-hz0-3-gamma0-2-dt0-05-steps160.png)
...
![Closed vs open staggered $Z$](./figures/stationary-chain-staggered-z-n10-jz1-hz0-3-gamma0-2-dt0-05-steps160.png)
...
![Final $Z$ profile at $t=8$](./figures/stationary-chain-final-profile-n10-jz1-hz0-3-gamma0-2-dt0-05-steps160.png)
...
![Trace preservation](./figures/stationary-chain-trace-n10-jz1-hz0-3-gamma0-2-dt0-05-steps160.png)
